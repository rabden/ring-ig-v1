...
import { calculateDimensions, handleApiResponse } from '@/utils/imageGenerationUtils'
...
export const useImageGeneration = ({
  ...
  const generateImage = async (retryCount = 0) => {
    ...
    const modifiedPrompt = `${prompt} ${styleSuffix} ${modelConfigs[model]?.promptSuffix || ''}`.trim();
