...
const TabsList = React.forwardRef(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      "inline-flex h-9 items-center justify-center rounded-xl p-1 text-muted-foreground/60",
      className
    )}
    {...props} />
))
...
const TabsTrigger = React.forwardRef(({ className, ...props }, ref) => (
  <TabsPrimitive.Trigger
    ...
    className={cn(
      ...
      "hover:bg-muted/10 hover:text-accent-foreground",
      "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
      "disabled:pointer-events-none disabled:opacity-40",
      "data-[state=active]:bg-muted/20 data-[state=active]:text-foreground/90 data-[state=active]:shadow-sm",
      className
    )}
    {...props} />
))
